    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div class="flex items-center p-4 justify-between">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm font-semibold text-gray-800"><?php echo $r['author_name']?></p>
        ...
        <div class="p-4">
            ...
            <?php if (!empty($r['imageArt'])): ?>
                <img src="../../assets/image/<?php echo htmlspecialchars($r['imageArt']); ?>" 
        ...
        <form 
            onsubmit="submitComment(event, <?php echo htmlspecialchars($r['idArticle']); ?>)" 
            method="POST" 
            action="add_comment.php" 
            class="flex flex-col gap-4"
        >
            <textarea 
                id="commentInput<?php echo htmlspecialchars($r['idArticle']); ?>" 
                name="comments" 
                rows="3" 
            ...
            <input type="hidden" name="articleId" value="<?php echo htmlspecialchars($r['idArticle']); ?>">
            <input type="hidden" name="user_id" value="<?php echo $_SESSION['id_user']; ?>">
            <div class="flex justify-end gap-4">
                <button 
                    type="button" 
                    class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 transition"
                    onclick="toggleCommentInput(<?php echo htmlspecialchars($r['idArticle']); ?>)"
                >
                    Cancel
                </button>
                <button 
                    type="submit" name="submitcomment"
...
<script>
    ...
    function toggleCommentInput(articleId) {
        const commentSection = document.getElementById('commentSection' + articleId);
        if (commentSection) {
            commentSection.classList.toggle('hidden');
        } else {
            console.error('Comment section not found for article ID: ' + articleId);
        }
    }
