...
<body style="scroll-behavior: smooth;">
                ...
                <div class="space-y-6">
                    <div>
                        <input type="text" id="searchInput"
                            ...
                            placeholder="Search by model..." onkeyup="filterVehicles()">
                    </div>
                ...
                <div class="space-y-12" id="vehicleList">
                    <?php foreach ($categories as $category): ?>
                        <div class="category-section mb-8">
                            ...
                            <p class="text-gray-600 mb-8 text-lg"><?php echo htmlspecialchars($category['description']); ?>
                            </p>
                                ...
                                <?php if (isset($categoryVehicles[$category['id_category']]) && count($categoryVehicles[$category['id_category']]) > 0): ?>
                                    <?php foreach ($categoryVehicles[$category['id_category']] as $vehicle): ?>
                                        <div class="vehicle-card bg-white p-4 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105 relative"
                                            data-vehicle-id="<?php echo $vehicle['id_vehicle']; ?>">
                                            <img src="../assets/image/<?php echo htmlspecialchars($vehicle['imageVeh']); ?>"
                                                alt="<?php echo htmlspecialchars($vehicle['model']); ?>"
                                                class="w-full h-48 object-cover rounded-t-lg mb-4">

                                            <div class="mt-4 flex flex-col items-center justify-center">
                                                <h4 class="text-xl font-semibold text-gray-800">
                                                    <?php echo htmlspecialchars($vehicle['model']); ?></h4>
                                                <p class="text-sm text-gray-500">Transmission:
                                                    <?php echo htmlspecialchars($vehicle['transmissionType']); ?> | Mileage:
                                                    <?php echo htmlspecialchars($vehicle['mileage']); ?>/Km</p>
                                                <p class="mt-2 text-gray-600">Fuel:
                                                    <?php echo htmlspecialchars($vehicle['fuelType']); ?></p>
                                                <p class="mt-2 text-gray-600">Price:
                                                    $<?php echo number_format($vehicle['price_per_day'], 2); ?>/day </p>

                                                <button id="details"
                                                    class="mt-4 mx-auto bg-blue-500 hover:bg-blue-700 text-white py-2 px-12 md:px-15 md:py-1 rounded-md transition duration-200 transform hover:scale-105">
                                                    View Details
                                                </button>
                                            </div>
                                        </div>
                        ...
                        <ul class="flex space-x-2">
                            ...
                            <?php if ($currentPage > 1): ?>
                                <li><a href="?page=<?php echo $currentPage - 1; ?>"
                                        class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg">Previous</a></li>
                            <?php endif; ?>

                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                <li>
                                    <a href="?page=<?php echo $i; ?>"
                                        class="px-4 py-2 <?php echo ($i == $currentPage) ? 'bg-blue-500 text-white' : 'bg-gray-200 hover:bg-gray-300'; ?> rounded-lg">
                                        <?php echo $i; ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
                            ...
                            <?php if ($currentPage < $totalPages): ?>
                                <li><a href="?page=<?php echo $currentPage + 1; ?>"
                                        class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg">Next</a></li>
                            <?php endif; ?>
                        </ul>
                    </nav>
                </div>
    ...
    <div id="reservationsPage" class="page max-w-7xl mx-auto p-6 bg-gray-50">
        ...
        <?php if ($rs): ?>
            <?php foreach ($rs as $r):
                ...
                <div id="reservationsList" class="mt-12 space-y-8">
                    <div
                        class="reservation-card bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105">
                        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                       <img
                        class="font-semibold w-80 h-80" src="../assets/image/<?php echo htmlspecialchars($r['imageVeh']); ?>">
                            <div class="flex-1">
                                <h3 class="text-2xl font-semibold text-gray-800">Car Model: <span
                                class="font-semibold"><?php echo htmlspecialchars($r['model']); ?></h3>
                                <p class="text-sm text-gray-500 mt-2">Reservation Start Date: <span
                                        class="font-semibold"><?php echo htmlspecialchars($r['start_date']); ?></span></p>
                                <p class="text-sm text-gray-500 mt-2">Reservation End Date: <span
                                        class="font-semibold"><?php echo htmlspecialchars($r['end_date']); ?></span></p>
                                <p class="mt-2 text-gray-600">PickUp location: <span
                                        class="font-semibold"><?php echo htmlspecialchars($r['pickup_location']); ?></span></p>
                                <p class="mt-2 text-gray-600">DropOff location : <span
                                        class="font-semibold"><?php echo htmlspecialchars($r['dropoff_location']); ?></span></p>
                                        <?php 
                                echo '<td class="border p-4 text-center text-sm ' . $statusClass . ' rounded-lg font-semibold>' . $r['status'] . '</td>';
                                ?>
                                
                                       
                                <div class="flex gap-4 mt-6">
                                    <button
                                        class="bg-blue-600 text-white py-2 px-6 rounded-md hover:bg-blue-700 transition duration-200 transform hover:scale-105">
                                        Modify Reservation
                                    </button>
                                    <button
                                        class="bg-red-600 text-white py-2 px-6 rounded-md hover:bg-red-700 transition duration-200 transform hover:scale-105">
                                        Cancel Reservation
                                    </button>
                                </div>
                            </div>
        ...
        <?php else: ?>
            <p class="text-gray-600 text-lg">No Reservation available.</p>
        <?php endif; ?>
